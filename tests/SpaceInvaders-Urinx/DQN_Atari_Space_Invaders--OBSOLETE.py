...
class DQNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='DQNetwork'):
        with tensorflow.compat.v1.variable_scope(name):
            self.inputs = tensorflow.compat.v1.placeholder(tensorflow.float32, [None, *state_size], name='inputs')
            self.actions = tensorflow.compat.v1.placeholder(tensorflow.float32, [None, action_size], name='actions')
            self.target_q = tensorflow.compat.v1.placeholder(tensorflow.float32, [None], name='target_q')
            ...
            conv1 = tensorflow.layers.conv2d(
                ...
                strides = [4, 4],
                padding = 'VALID',
                kernel_initializer = tensorflow.contrib.layers.xavier_initializer_conv2d(),
                name = 'conv1'
            )
            conv1_out = tensorflow.nn.elu(conv1, name='conv1_out')
            ...
            conv2_out = tensorflow.nn.elu(conv2, name='conv2_out')
            ...
            conv3_out = tensorflow.nn.elu(conv3, name='conv3_out')

            flatten = tensorflow.contrib.layers.flatten(conv3_out)
            fc = tensorflow.layers.dense(
                inputs = flatten,
                units = 512,
                activation = tensorflow.nn.elu,
                kernel_initializer = tensorflow.contrib.layers.xavier_initializer(),
                name = 'fc'
            )
            self.output = tensorflow.layers.dense(
                inputs = fc,
                units = action_size,
                activation = None,
                kernel_initializer = tensorflow.contrib.layers.xavier_initializer(),
            ...
            self.q = tensorflow.reduce_sum(tf.multiply(self.output, self.actions))
            self.loss = tensorflow.reduce_mean(tf.square(self.target_q - self.q))
            self.optimizer = tensorflow.train.AdamOptimizer(learning_rate).minimize(self.loss)
...
def train():
    ...
    writer = tensorflow.summary.FileWriter('train_log')
    tensorflow.summary.scalar('Loss', DQN.loss)
